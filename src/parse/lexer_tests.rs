...
fn assert_identifier(stream: &mut TokenStream, val: &str) { 
    assert_eq!(stream.next().unwrap().as_identifier().unwrap().node, val); 
}
...
fn words() { 
    ...
    assert_eq!(token.as_identifier().unwrap().node, "asdf"); 
    assert_eq!(token.span, Span { lo: Position(0), hi: Position(4) });

    let token = stream.next().unwrap(); 
    assert_eq!(token.as_identifier().unwrap().node, "jkl"); 
    assert_eq!(token.span, Span { lo: Position(5), hi: Position(8) });

    let token = stream.next().unwrap(); 
    assert_eq!(token.as_identifier().unwrap().node, "ree"); 
...
fn line_comment() -> ParseResult<()> { 
    ...
    assert_eq!(next.as_identifier().unwrap().node, "asdf");
    assert_eq!(next.span, Span { lo: Position(0), hi: Position(4) });

    let next = stream.next().unwrap();
    assert_eq!(next.as_identifier().unwrap().node, "foobar");
    assert_eq!(next.span, Span { lo: Position(31), hi: Position(37) });

    let next = stream.next().unwrap();
    assert_eq!(next.as_identifier().unwrap().node, "endofline");
...
fn block_comment() { 
    ...
    let next = stream.next().unwrap();
    assert_eq!(next.as_identifier().unwrap().node, "end");
...
fn spacing() { 
    ...
    let next = stream.next().unwrap();
    assert_eq!(next.as_identifier().unwrap().node, "aaa");
    assert_eq!(next.span, Span { lo: Position(0), hi: Position(3) });

    let next = stream.next().unwrap();
    assert_eq!(next.as_identifier().unwrap().node, "eee");
    assert_eq!(next.span, Span { lo: Position(4), hi: Position(7) });

    let next = stream.next().unwrap();
    assert_eq!(next.as_identifier().unwrap().node, "foo");
    assert_eq!(next.span, Span { lo: Position(14), hi: Position(17) });

    let next = stream.next().unwrap();
    assert_eq!(next.as_identifier().unwrap().node, "bar");
